...
open Patch_processor.Expression
...
let ve_testable =
  let pp_ve fmt ve =
    Format.fprintf fmt "{ file_name = %s; line_no = %d; before_context = %s; after_context = %s; original_expression = %s; modified_expression = %s }"
      ...
      ve.before_context
      ve.after_context
      ve.original_expression
      ve.modified_expression
  in
  Alcotest.testable pp_ve (=)

let test_diff ~file_name ~line_no ~before_context ~after_context ~original_lines ~modified_lines ~before_context_expr ~after_context_expr  ~original_expression ~modified_expression expected_name =
  let diff = {
    file_name;
    line_no;
    before_context;
  ...
  let expected = {
    file_name;
    line_no;
    before_context = before_context_expr;
    after_context = after_context_expr;
    original_expression;
    modified_expression;
  } in
  let result = reduce_diff_to_expression diff "max" in
  Alcotest.(check ve_testable) expected_name expected result
